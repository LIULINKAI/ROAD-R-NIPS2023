...
class ViT_CLIP(nn.Module):
    ...
    def forward(self, x: torch.Tensor, boxes_embedding):
        B, C, T, H, W = x.shape
        ...
        boxes_embedding = rearrange(boxes_embedding, 'b t n d -> (b t) n d')
        if self.use_local:
            x = torch.cat([x, x_local, boxes_embedding], dim=1)
        else:
            x = torch.cat([x, boxes_embedding], dim=1)
        n = x.shape[1]
...
class ViTCLIPClassifier_location_pro(nn.Module):
    def __init__(self,
...
if __name__ == "__main__":
    ...
    for name, param in model.named_parameters():
        if 'bbox_embedding' not in name and 'temporal_embedding' not in name and 'ln_post' not in name and 'cls_head' not in name and 'Adapter' not in name:
    ...
    inputs = torch.rand(size=(2, 6, 4, 224, 224)).to(device)
    optimizer = optim.Adam(model.parameters(), lr=0.01)
    model.train()
    output = model(inputs, torch.rand(size=(2, 4, 4)).to(device))
    print(output)
    print(output.shape)