...
from tools.stage_two_opt import arg_parse
...
def model_init(args):
    ...
    for name, param in model.named_parameters():
        if 'location_embedding' not in name and 'agent_embedding' not in name and 'bbox_embedding' not in name and 'temporal_embedding' not in name and 'ln_post' not in name and 'cls_head' not in name and 'Adapter' not in name:
...
def main():
    ...
    train_set = mydataset(args, is_train=True, use_local=args.use_local)
    valid_set = mydataset(args, is_train=False, use_local=args.use_local)
