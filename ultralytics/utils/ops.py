...
def non_max_suppression(
    ...
    output_cls_conf = [torch.zeros((0, nc), device=prediction.device)] * bs
...
# --------------------------------- TODO 修改版本如下---------------------------------------
def non_max_suppression_predict(
        prediction,
        conf_thres=0.25,
        iou_thres=0.45,
        classes=None,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        output_cls_conf[xi] = cls[i]
        if mps:
            output[xi] = output[xi].to(device)
            output_cls_conf[xi] = output_cls_conf[xi].to(device)
        if (time.time() - t) > time_limit:
            LOGGER.warning(f'WARNING ⚠️ NMS time limit {time_limit:.3f}s exceeded')
            break  # time limit exceeded
    return output, output_cls_conf
